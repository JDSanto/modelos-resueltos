...
Se trata de un problema de planificación de producción, considerando una mezcla para producir las promos.
...
- No hay inflación.
- Se pueden producir cantidades arbitrariamente pequeñas de dulces.
- Las promociones pueden estar formadas por proporciones de tipos de dulces, con tal de que en total sean la cantidad que le corresponda a cada promoción.
- El costo de producción de cada dulce es directamente proporcional a la cantidad producida.
...
- $P_d$ [$unidad/mes$]: Cantidad de $d$ a producir en el mes.
  - Donde $d \in \{alfa, beta, gama, barra\}$.
- $P_p$ [$promo/mes$]: Cantidad de promoción $p$ a producir en el mes.
  - Donde $p \in \{1, 2\}$.
- $C_{barra}$ [$unidad/mes$]: Cantidad de barritas a comprar en el mes.
- $M_{p,d}$ [$unidad/promo$]: Cantidad de unidades de $d$ para utilizar en la producción de promo $p$.

Se definen también las siguientes variables:

- $P_{1,1}$ [$unidades/mes$]: Cantidad de promo 1 a producir (hasta 8 unidades, inclusive) en el mes.
- $P_{1,2}$ [$unidades/mes$]: Cantidad de promo 1 a producir (pasadas las 8 unidades) en el mes.
...
- $MP$ [$unidades/mes$]: Toma el menor valor entre las promociones $P_1$ y $P_2$.
- $Y_{p,menor}$ [binaria]: Toma valor 1 si la cantidad a producir de la promoción $p$ es la menor de las promociones.
...
$$
2 \cdot P_1 = P_{barra} + C_{barra}
$$
$$
P_2 = P_{barra} + C_{barra}
$$

La restricción del máximo de 330 dulces con la línea de producción, vinculando con la variable bivalente $Y_{330dulces}$ (utilizando $m$ con un valor "muy chico"):
$$
330 \cdot Y_{330dulces} \le P_{alfa} + P_{beta} + P_{gama} \le 330 \cdot Y_{330dulces} + (330 - m) \cdot (1 - Y_{330dulces})
$$
...
$$
Y_{benef} \cdot SP \le P_1 + P_2 \le (1 - Y_{benef}) \cdot (SP - m) + Y_{benef} \cdot M
$$
...
Es necesario agregar una nueva variable:
...
Se vincula con la variable de producción y $Y_{2,menor}$:
$$
BENEF_{1,premio} \le P_1
$$
$$
BENEF_{1,premio} \le Y_{2,menor} \cdot M
$$
...
$$
$$
+ Y_{1,menor} \cdot 300 + BENEF_{1,premio} \cdot 15
$$
