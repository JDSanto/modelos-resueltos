...
Calcular la cantidad de productos A, B y C a comprar para maximizar la ganancia por venta de productos en el plazo de los datos.
...
- $C_{C,c}$ [$unidad/T$]: Cantidad del producto $C$ a comprar en el plazo $T$, si dicha cantidad se encuentra en el intervalo $c$, donde:
  - $b = 1$ para el intervalo $[0, 1000)$
  - $b = 2$ para el intervalo $[1000, 3000]$
  - $b = 3$ para el intervalo $[3000, \inf)$
...
- Ventas de $B$ menores a $C$:
$$
-M \cdot I_{B,MENOR} \cdot unidad/T \le V_B - V_C \le M \cdot (1 - I_{B,MENOR}) \cdot unidad/T
$$
$$
V_A \le V_B + I_{B,MENOR} \cdot M
$$
$$
V_A \le V_C + I_{B,MENOR} \cdot M
$$
